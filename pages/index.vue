<template>
    <div class="flex w-full flex-col items-center">
        ...
        <div class="my-8 flex w-full max-w-4xl flex-col">
            ...
            <template v-else>
                ...
                <div v-else class="md:border-l md:border-gray-100">
                    <div class="flex flex-col space-y-4 md:pl-6">
                        <article v-for="article in articlesResponse.items" :key="article.id"
            ...
            <nav v-if="articlesResponse" class="mt-12 flex items-center justify-between px-4 py-3 sm:px-6">
                <div class="flex flex-1 justify-center sm:justify-end">
                    <NuxtLink v-if="currentPage > 1"
                        class="flex items-center text-xl font-medium text-gray-600 hover:text-emerald-500" :to="{
                            name: 'index',
                            query: {
                                page: currentPage - 1
                            }
                    ...
                    <NuxtLink class="flex items-center text-xl font-medium text-gray-600 hover:text-emerald-500" :to="{
                        name: 'index',
                        query: {
                            page: currentPage + 1
                        }
...
<script setup>
    // 取得前端URL路徑資訊: 預設資料型別是字串, 後續要加工轉換成 number
